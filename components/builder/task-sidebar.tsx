...
interface TaskSidebarProps {
  tasks: BuilderTask[]
  onTaskSelect: (task: BuilderTask) => void
  width?: number
}
...
export function TaskSidebar({ tasks, onTaskSelect, width = 288 }: TaskSidebarProps) {
  ...
  const handleDeleteTasks = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/builder/tasks?action=${actions.join(',')}`, {
        method: 'DELETE',
      })
      ...
      <div className="mb-3 md:mb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link href="/builder" onClick={handleNewTaskClick}>
      ...
      {activeTab === 'tasks' && (
        <div className="space-y-1">
          {tasks.length === 0 ? (
            ...
            tasks.map((task) => {
              const isActive = pathname === `/builder/tasks/${task.id}`

              return (
                <Link
                  key={task.id}
                  href={`/builder/tasks/${task.id}`}
        ...
        <div className="space-y-1">
          {repositories.length === 0 ? (
            ...
            repositories.map((repo) => {
              const repoPath = `/builder/repos/${repo.owner}/${repo.name}`
