...
export function MergePRDialog({
  ...
  const handleMergePR = async () => {
    ...
    try {
      const response = await fetch(`/api/builder/tasks/${taskId}/merge-pr`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && result.success) {
        onPRMerged?.();
        onOpenChange(false);
      } else if (result.error && (result.error.includes("conflict") || result.error.includes("mergeable"))) {
      ...
      >
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              This pull request has merge conflicts that prevent automatic merging. Would you like an AI agent to resolve
              the conflicts for you?
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
