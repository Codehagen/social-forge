...
EOF`;
  ...
  return {
    ...
    if (agentMessageId && result.output) {
      await updateTaskMessageContent(agentMessageId, result.output);
    } else if (taskId && result.output) {
      await createTaskMessage(taskId, BuilderTaskMessageRole.AGENT, result.output);
    }
