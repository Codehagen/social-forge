...
import { parseMorphEdits, applyMorphEditToFile } from '@/lib/lovable/morph-fast-apply';
// Sandbox import not needed - using global sandbox from sandbox-manager
import type { SandboxState } from '@/types/lovable/sandbox';
import type { ConversationState } from '@/types/lovable/conversation';
import { sandboxManager } from '@/lib/lovable/sandbox/sandbox-manager';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!provider) {
      ...
      try {
        const { SandboxFactory } = await import('@/lib/lovable/sandbox/factory');
